...
const { runInterviewAdvice } = require('./interview-service');
...
function errorDetails(error) {
  ...
  const messages = {
    ...
    MODEL_JSON_SCHEMA_INVALID: '模型返回结构异常',
...
async function releaseUserTask(openid, taskId) {
  ...
  await withTimeout(
    db.collection('users')
      .where({ openid, active_interview_task_id: taskId })
      .update({ active_interview_task_id: null }),
    3000,
...
async function finalizeSuccessfulTask(task, workerId, diagnosis) {
  // task_id is the canonical interview_task document ID. Do not depend on
  ...
  await db.runTransaction(async transaction => {
    const transactionTaskColl = transaction.collection('interview_task');
    ...
    const ledgerResult = await transaction.collection('interview_quota_ledger')
    ...
    let quotaType;
    if (Number(user.interviewFreeLeft || 0) > 0) quotaType = 'free';
    else if (Number(user.payInterviewCount || 0) > 0) quotaType = 'paid';
    ...
    const resultData = {
      ...
      status: diagnosis.parsedResult.status,
      brief_summary: diagnosis.parsedResult.brief_summary,
      profileOverview: diagnosis.parsedResult.profileOverview,
      keyExaminePoint: diagnosis.parsedResult.keyExaminePoint,
      matchAdvice: diagnosis.parsedResult.matchAdvice,
      interviewSkillGuide: diagnosis.parsedResult.interviewSkillGuide,
      questionList: diagnosis.parsedResult.questionList,
      create_time: db.serverDate(),
      time_to_first_token_sec: diagnosis.timeToFirstTokenSec,
      total_model_cost_sec: diagnosis.totalModelCostSec
    };

    await transaction.collection('interview_result').doc(resultDocumentId).set(resultData);
    await transaction.collection('users').doc(user._id).update({
      ...(quotaType === 'free'
        ? { interviewFreeLeft: cmd.inc(-1) }
        : { payInterviewCount: cmd.inc(-1) }),
      active_interview_task_id: null
    });
    await transaction.collection('interview_quota_ledger').doc(task.task_id).set({
      task_id: task.task_id,
      openid: task.openid,
      status: 'charged',
...
async function finishInvalidInput(task, workerId, result) {
  const now = Date.now();
  const updateResult = await withTimeout(
    taskColl.where({ _id: task._id, status: 'running', worker_id: workerId }).update({
      status: 'failed',
      stage: 'failed',
      error_code: String(result.status || 'INPUT_INVALID').toUpperCase(),
      error_message: result.brief_summary,
      input: null,
      retryable: true,
      charged: false,
      worker_id: null,
...
async function processTask(task, workerId) {
  ...
  try {
    const diagnosis = await runInterviewAdvice(task.input, {
    ...
    await scheduleFlush();
    await flushChain;
    if (diagnosis.parsedResult.status !== 'ok') {
      clearInterval(leaseTimer);
      await finishInvalidInput(task, workerId, diagnosis.parsedResult);
      return;
    }
    ...
    console.error(`【面试Worker失败】taskId=${task.task_id}`, error);
  } finally {
...
async function runWorkerLoop(workerId) {
  console.log(`【面试Worker启动】workerId=${workerId}`);
