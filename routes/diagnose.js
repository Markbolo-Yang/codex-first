...
const resultColl = db.collection('diagnose_result');

function createTaskId() {
  // 控制在32字符以内，可直接作为CloudBase文档ID用于任务、结果和扣费流水幂等。
  return `diag_${Date.now().toString(36)}_${crypto.randomBytes(8).toString('hex')}`;
}

function createDiagnoseId() {
...
function validateCreateBody(body) {
  const { openid, resumeInfo, jobTarget, customPrompt, resume_id } = body || {};
  if (!openid || !resumeInfo) {
  ...
  return {
    ...
    input: {
      resumeInfo: safeInputValue(resumeInfo),
      jobTarget: safeInputValue(jobTarget),
...
async function createOrGetTask(body) {
  ...
  return db.runTransaction(async transaction => {
    ...
    const totalQuota = Number(user.resumeFreeLeft || 0) + Number(user.payResumeCount || 0);
    if (totalQuota <= 0) throw new Error('COUNT_EXHAUST');

    if (user.active_diagnose_task_id) {
      const activeResult = await transaction.collection('diagnose_task')
        .doc(user.active_diagnose_task_id)
        .get();
      const activeTask = activeResult.data?.[0];
    ...
    await transaction.collection('diagnose_task').doc(taskId).set(task);
    await usersColl.doc(user._id).update({ active_diagnose_task_id: taskId });
...
async function getTaskResult(task) {
  ...
  return {
    diagnose_id: document.diagnose_id,
    resume_id: document.resume_id,
    overview: document.overview,
    advantages: document.advantages,
    improve: document.improve,
    optimize_ref: document.optimize_ref,
    final_tips: document.final_tips
  };
}
...
function createErrorResponse(error) {
  ...
  if (code === 'PARAM_ERROR') return { status: 400, body: { code, msg: '缺少openid或简历参数' } };
  if (code === 'USER_NOT_FOUND') return { status: 404, body: { code, msg: '用户不存在', noDeduct: true } };
  if (code === 'COUNT_EXHAUST') {
    return { status: 409, body: { code, msg: '诊断次数已用尽，请购买或领取次数后再试', noDeduct: true, retryable: false } };
  }
  return { status: 500, body: { code: 'TASK_CREATE_FAILED', msg: '诊断任务创建失败，本次未扣次数', noDeduct: true } };
}
...
router.get('/tasks/:taskId', async (req, res) => {
  try {
    ...
    if (!task) return res.status(404).json({ code: 'TASK_NOT_FOUND', msg: '诊断任务不存在' });
    ...
    console.error('【诊断任务查询失败】', error);
    return res.status(500).json({ code: 'TASK_QUERY_FAILED', msg: '诊断进度查询失败，请稍后重试' });
  }
});
  ...
  try {
    ...
    if (task.status === 'queued') {
      await db.runTransaction(async transaction => {
        const taskResult = await transaction.collection('diagnose_task').doc(task._id).get();
        const currentTask = taskResult.data?.[0];
        if (!currentTask || currentTask.status !== 'queued') return;
        await transaction.collection('diagnose_task').doc(task._id).update({
          status: 'cancelled',
          stage: 'cancelled',
    ...
    console.error('【取消诊断任务失败】', error);
    return res.status(500).json({ code: 'TASK_CANCEL_FAILED', msg: '取消诊断失败，请稍后重试' });
  }
});
      ...
      relayTimer = setInterval(async () => {
        ...
        try {
          ...
          if (!res.writableEnded) {
            res.write(`data: ${JSON.stringify({ code: 'TASK_QUERY_FAILED', msg: '诊断进度查询失败', noDeduct: true })}\n\n`);
            res.end();
          }
          clearInterval(relayTimer);
          req.socket.off('close', closeHandler);
