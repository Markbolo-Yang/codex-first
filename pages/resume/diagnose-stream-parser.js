...
function advanceReportDisplay(currentValue, targetValue, charBudget = 8) {
  ...
  next.improve.base_info = growStringList(current.improve.base_info, target.improve.base_info, budget);
  ...
  for (let index = 0; index < target.optimize_ref.length && budget.left > 0; index += 1) {
    ...
    const currentReference = currentReferences[index] || {
      project_name: '', original_text: '', optimize_content: ''
    };
    const reference = {
      ...
      optimize_content: growText(currentReference.optimize_content, targetReference.optimize_content, budget)
    };
  ...
  return { report: next, done: JSON.stringify(next) === JSON.stringify(target) };
}
